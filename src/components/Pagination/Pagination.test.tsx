...
describe('Pagination', () => {
	it('Render Ophan Data Components as expected', () => {
		...
		expect(
			asFragment().querySelectorAll('[data-link-name="Pagination view page 2"]')
				.length,
		).toBe(1); // The number is still a button

		expect(
			asFragment().querySelectorAll('[data-link-name="Pagination view page 1"]')
				.length,
		).toBe(2); // Both the number and the back button

		expect(
			asFragment().querySelectorAll('[data-link-name="Pagination view page 3"]')
				.length,
		).toBe(2); // Both the number and the forward button

		expect(
			asFragment().querySelectorAll('[data-link-name="Pagination view page 5"]')
				.length,
		).toBe(0); // Page five should be hidden

		expect(
			asFragment().querySelectorAll('[data-link-name="Pagination view page 9"]')
				.length,
